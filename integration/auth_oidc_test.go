...
func TestOIDCAuthenticationPingAll(t *testing.T) {
	...
	oidcMap := map[string]string{
		"NINJAPANDA_OIDC_ISSUER":             oidcConfig.Issuer,
		"CREDENTIALS_DIRECTORY_TEST":         "/tmp",
		"NINJAPANDA_OIDC_CLIENT_SECRET_PATH": "${CREDENTIALS_DIRECTORY_TEST}/hs_client_oidc_secret",
		"NINJAPANDA_OIDC_CLIENT_SECRET":      oidcConfig.ClientSecret,
		"NINJAPANDA_OIDC_STRIP_EMAIL_DOMAIN": fmt.Sprintf(
			"%t",
...
func TestOIDCExpireNodes(t *testing.T) {
	...
	oidcMap := map[string]string{
		"NINJAPANDA_OIDC_ISSUER":        oidcConfig.Issuer,
		"NINJAPANDA_OIDC_CLIENT_ID":     oidcConfig.ClientID,
		"NINJAPANDA_OIDC_CLIENT_SECRET": oidcConfig.ClientSecret,
		"NINJAPANDA_OIDC_STRIP_EMAIL_DOMAIN": fmt.Sprintf(
			"%t",
			oidcConfig.StripEmaildomain,
...
func (s *AuthOIDCScenario) CreateNinjapandaEnv(
...
func (s *AuthOIDCScenario) runMockOIDC(
	...
	hash, _ := ninjapanda.GenerateRandomStringDNSSafe(hsicOIDCMockHashLength)

	hostname := fmt.Sprintf("np-oidcmock-%s", hash)

	mockOidcOptions := &dockertest.RunOptions{
		Name:         hostname,
		Cmd:          []string{"ninjapanda", "mockoidc"},
	...
	log.Println("Waiting for ninjapanda mock oidc to be ready for tests")
	...
	if err := s.pool.Retry(func() error {
		...
		if err != nil {
			log.Printf("ninjapanda mock OIDC tests is not ready: %s\n", err)
	...
	log.Printf("ninjapanda mock oidc is ready for tests at %s", hostEndpoint)

	return &ninjapanda.OIDCConfig{
		Issuer: fmt.Sprintf(
			"http://%s/oidc",
...
func (s *AuthOIDCScenario) runClientUp(
	...
	if namespace, ok := s.namespaces[namespaceStr]; ok {
		for _, client := range namespace.Clients {
			...
			go func(c ZTClient) {
				...
				log.Printf("Finished request for %s to join ztnet", c.Hostname())
			}(client)
...
func pingAll(t *testing.T, clients []ZTClient, ips []netip.Addr) int {
	t.Helper()
	success := 0
