...
var mockOidcCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		err := mockOIDC()
		if err != nil {
			log.Error().Caller().Err(err).Msgf("Error running mock OIDC server")
			os.Exit(1)
		}
	},
}
...
func mockOIDC() error {
	...
	log.Info().
		Caller().
		Msgf("Mock OIDC server listening on %s", listener.Addr().String())
	log.Info().Caller().Msgf("Issuer: %s", mock.Issuer())
	c := make(chan struct{})
	<-c
