...
func init() {
	...
	expireNodeCmd.Flags().StringP("identifier", "i", "", "Node identifier (MachineId)")
	...
	renameNodeCmd.Flags().StringP("identifier", "i", "", "Node identifier (MachineId)")
	...
	deleteNodeCmd.Flags().StringP("identifier", "i", "", "Node identifier (MachineId)")
	...
	moveNodeCmd.Flags().StringP("identifier", "i", "", "Node identifier (MachineId)")
	...
	tagCmd.Flags().StringP("identifier", "i", "", "Node identifier (MachineId)")
...
var nodeCmd = &cobra.Command{
	Use:     "nodes",
	Short:   "Manage the nodes of Ninjapanda",
...
func nodesToPtables(
	...
	for _, machine := range machines {
		...
		if machine.LastSeen != nil {
			d := ninjapanda.ParseTime(*machine.LastSeen)
			lastSeen = d.AsTime()
			lastSeenTime = lastSeen.Format(time.RFC3339)
		}

		var expiry time.Time
		var expiryTime string
		if machine.Expiry != nil {
			d := ninjapanda.ParseTime(*machine.Expiry)
			expiry = d.AsTime()
		...
		err := machineKey.UnmarshalText(
			[]byte(ninjapanda.MachinePublicKeyEnsurePrefix(machine.MachineKey)),
		)
		if err != nil {
		...
		err = nodeKey.UnmarshalText(
			[]byte(ninjapanda.NodePublicKeyEnsurePrefix(machine.NodeKey)),
		)
		if err != nil {
			return nil, err
		}
...
var tagCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			ErrorOutput(
				err,
				fmt.Sprintf("Error while sending tags to ninjapanda: %s", err),
				output,
			)
