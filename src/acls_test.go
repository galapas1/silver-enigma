...
func (s *Suite) TestBrokenHuJson(c *check.C) {
	err := app.LoadACLPolicy("../tests/acls/broken.json")
	c.Assert(err, check.NotNil)
}

func (s *Suite) TestInvalidPolicyHuson(c *check.C) {
	err := app.LoadACLPolicy("../tests/acls/invalid.json")
	c.Assert(err, check.NotNil)
...
func (s *Suite) TestBasicRule(c *check.C) {
	err := app.LoadACLPolicy("../tests/acls/acl_policy_basic_1.json")
	c.Assert(err, check.IsNil)

	rules := generateACLRules([]Machine{}, *app.aclPolicy, false)
	c.Assert(err, check.IsNil)
	c.Assert(rules, check.NotNil)
}

func (s *Suite) ignoreTestDenyActionSinglePort(c *check.C) {
	var hosts Hosts
	hosts.UnmarshalJSON(
		[]byte(
			`{"example-host-1": "100.100.100.100","example-host-2": "100.100.101.100/24"}`,
	...
	app.aclPolicy = &ACLPolicy{
		...
		ACLs: []ACL{
			{
				Action:       "accept",
				Sources:      []string{"example-host-2"},
				Protocol:     "tcp",
				Destinations: []string{"*:*"},
			},
			{
				Action:       "deny",
				Sources:      []string{"example-host-1"},
				Protocol:     "tcp",
				Destinations: []string{"*:3689"},
			},
		},
	}
...
func (s *Suite) TestSshRules(c *check.C) {
	envknob.Setenv("NINJAPANDA_EXPERIMENTAL_FEATURE_SSH", "1")
...
// TBD: This feature is under review as the API evolves.
func (s *Suite) tbdTestInvalidGroupInGroup(c *check.C) {
...
// TBD: This feature is under review as the API evolves.
func (s *Suite) tbdTestInvalidTagOwners(c *check.C) {
...
func (s *Suite) tbdTestValidTagInvalidNamespace(c *check.C) {
	namespace, err := app.CreateNamespace("user1")
	c.Assert(err, check.IsNil)
...
func (s *Suite) TestPortRange(c *check.C) {
	err := app.LoadACLPolicy("../tests/acls/acl_policy_basic_range.json")
	c.Assert(err, check.IsNil)
...
func (s *Suite) TestProtocolParsing(c *check.C) {
	err := app.LoadACLPolicy(
		"../tests/acls/acl_policy_basic_protocols.json",
	)
	c.Assert(err, check.IsNil)
...
func (s *Suite) TestPortWildcard(c *check.C) {
	err := app.LoadACLPolicy(
		"../tests/acls/acl_policy_basic_wildcards.json",
	)
	c.Assert(err, check.IsNil)
...
func (s *Suite) TestPortNamespace(c *check.C) {
	...
	err = app.LoadACLPolicy(
		"../tests/acls/acl_policy_basic_namespace_as_user.json",
	)
	c.Assert(err, check.IsNil)
	...
	rules := generateACLRules(machines, *app.aclPolicy, false)
	c.Assert(err, check.IsNil)
	c.Assert(rules, check.NotNil)
...
func (s *Suite) TestPortGroup(c *check.C) {
	...
	err = app.LoadACLPolicy("../tests/acls/acl_policy_basic_groups.json")
	c.Assert(err, check.IsNil)
