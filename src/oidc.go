...
func (np *Ninjapanda) initOIDC() error {
	var err error
	// grab oidc config if it hasn't been already
	if np.oauth2Config == nil {
		np.oidcProvider, err = oidc.NewProvider(
			context.Background(),
			np.cfg.OIDC.Issuer,
		)
		...
		np.oauth2Config = &oauth2.Config{
			ClientID:     np.cfg.OIDC.ClientID,
			ClientSecret: np.cfg.OIDC.ClientSecret,
			Endpoint:     np.oidcProvider.Endpoint(),
			RedirectURL: fmt.Sprintf(
				"%s/oidc/callback",
				strings.TrimSuffix(np.cfg.ServerURL, "/"),
			),
			Scopes: np.cfg.OIDC.Scope,
		}
	}
...
func (np *Ninjapanda) RegisterOIDC(
	...
	np.registrationCache.StoreState(
		req.Context(),
		stateStr,
	...
	extras := make([]oauth2.AuthCodeOption, 0, len(np.cfg.OIDC.ExtraParams))

	for k, v := range np.cfg.OIDC.ExtraParams {
		extras = append(extras, oauth2.SetAuthURLParam(k, v))
	}

	authURL := np.oauth2Config.AuthCodeURL(stateStr, extras...)
...
func (np *Ninjapanda) OIDCCallback(
	writer http.ResponseWriter,
	req *http.Request,
) {
	...
	rawIDToken, err := np.getIDTokenForOIDCCallback(
		req.Context(),
		writer,
		code,
		state,
	...
	idToken, err := np.verifyIDTokenForOIDCCallback(
		req.Context(),
		writer,
		rawIDToken,
	)
	...
	if err := validateOIDCAllowedDomains(writer, np.cfg.OIDC.AllowedDomains, claims); err != nil {
		return
	}

	if err := validateOIDCAllowedGroups(writer, np.cfg.OIDC.AllowedGroups, claims); err != nil {
		return
	}

	if err := validateOIDCAllowedUsers(writer, np.cfg.OIDC.AllowedUsers, claims); err != nil {
		return
	}

	nodeKey, machineExists, err := np.validateMachineForOIDCCallback(
		req.Context(),
		writer,
		state,
		claims,
	...
	log.Debug().Caller().Msg("Registering new machine after successful callback")

	namespace, err := np.findOrCreateNewNamespaceForOIDCCallback(
		writer,
		namespaceName,
	)
	if err != nil {
	...
	if err := np.registerMachineForOIDCCallback(req.Context(), writer, namespace, nodeKey, idToken.Expiry); err != nil {
		return
	}
...
func (np *Ninjapanda) getIDTokenForOIDCCallback(
	ctx context.Context,
	writer http.ResponseWriter,
	code, state string,
) (string, error) {
	oauth2Token, err := np.oauth2Config.Exchange(ctx, code)
	if err != nil {
		log.Error().
			Caller().
			Err(err).
	...
	log.Trace().
		Caller().
		Str(logtags.MakeTag("code"), code).
		Str(logtags.MakeTag("state"), state).
...
func (np *Ninjapanda) verifyIDTokenForOIDCCallback(
	ctx context.Context,
	writer http.ResponseWriter,
	rawIDToken string,
) (*oidc.IDToken, error) {
	verifier := np.oidcProvider.Verifier(&oidc.Config{ClientID: np.cfg.OIDC.ClientID})
...
func validateOIDCAllowedDomains(
	...
	if len(allowedDomains) > 0 {
		if at := strings.LastIndex(claims.Email, "@"); at < 0 ||
			...
			log.Error().
				Caller().
				Msg("authenticated principal does not match any allowed domain")
			writer.Header().Set("Content-Type", "text/plain; charset=utf-8")
			writer.WriteHeader(http.StatusBadRequest)
...
func validateOIDCAllowedUsers(
	...
	if len(allowedUsers) > 0 &&
		...
		log.Error().
			Caller().
			Msg("authenticated principal does not match any allowed user")
		writer.Header().Set("Content-Type", "text/plain; charset=utf-8")
		writer.WriteHeader(http.StatusBadRequest)
...
// on to registration.
func (np *Ninjapanda) validateMachineForOIDCCallback(
	ctx context.Context,
	writer http.ResponseWriter,
	state string,
	claims *IDTokenClaims,
	...
	nodeKeyIf, nodeKeyFound := np.registrationCache.SearchState(ctx, state)
	if !nodeKeyFound {
		log.Error().
			Caller().
	...
	if err != nil {
		log.Error().
			Caller().
			Str(logtags.GetTag(logtags.machine, "NodeKey"), nodeKeyFromCache).
			Bool(logtags.MakeTag("nodeKeyOK"), nodeKeyOK).
	...
	// exist, then this is a new machine and we will move
	// on to registration.
	machine, _ := np.GetMachineByNodeKey(nodeKey)
	...
	if machine != nil {
		...
		err := np.RefreshMachine(machine, expiry)
		if err != nil {
			log.Error().
				Caller().
				Err(err).
		...
		log.Debug().
			Str(logtags.GetTag(logtags.machine, "Hostname"), machine.Hostname).
			Str(logtags.GetTag(logtags.machine, "Expiry"), fmt.Sprintf("%v", expiry)).
		...
		if err := oidcCallbackTemplate.Execute(&content, oidcCallbackTemplateConfig{
			...
			log.Error().
				Caller().
				Err(err).
				Msg("Could not render OIDC callback template for type=reauthenticate")
...
func getNamespaceName(
	...
	if err != nil {
		log.Error().Caller().Err(err).Msgf("couldn't normalize email")
		writer.Header().Set("Content-Type", "text/plain; charset=utf-8")
		writer.WriteHeader(http.StatusInternalServerError)
...
func (np *Ninjapanda) findOrCreateNewNamespaceForOIDCCallback(
	writer http.ResponseWriter,
	namespaceName string,
) (*Namespace, error) {
	namespace, err := np.GetNamespace(namespaceName)
	if errors.Is(err, ErrNamespaceNotFound) {
		namespace, err = np.CreateNamespace(namespaceName)
		...
		if err != nil {
			log.Error().
				...
				Msg("could not create new namespace")
			writer.Header().Set("Content-Type", "text/plain; charset=utf-8")
			writer.WriteHeader(http.StatusInternalServerError)
...
func (np *Ninjapanda) registerMachineForOIDCCallback(
	ctx context.Context,
	writer http.ResponseWriter,
	namespace *Namespace,
	...
	if _, err := np.RegisterMachineFromAuthCallback(
		nodeKey.String(),
		namespace.Name,
		&expiry,
		RegisterMethodOIDC,
