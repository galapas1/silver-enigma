...
func (sc *IAMSASLClient) Step(challenge string) (string, error) {
	...
	case initMessage:
		if challenge != "" {
			sc.state = failed
			return "", fmt.Errorf(
				"challenge must be empty for initial request: %w",
				ErrBadChallenge,
			)
		}
	...
	case serverResponse:
		if challenge == "" {
			sc.state = failed
			return "", fmt.Errorf(
				"challenge must not be empty for server resposne: %w",
				ErrBadChallenge,
			)
		}
		...
		if err := json.NewDecoder(strings.NewReader(challenge)).Decode(&resp); err != nil {
			sc.state = failed
			return "", fmt.Errorf(
				"unable to process msk challenge response: %w",
				multierr.Combine(err, ErrFailedServerChallenge),
			)
		}
		...
		if resp.Version != supportedVersion {
			sc.state = failed
			return "", fmt.Errorf(
				"unknown version found in response: %w",
				ErrFailedServerChallenge,
			)
		}
