...
var signUserAgent = "optm/sasl/aws_msk_iam_v2/" + runtime.Version()
...
func (m *Mechanism) Start(
	ctx context.Context,
) (sess sasl.StateMachine, ir []byte, err error) {
...
func (m *Mechanism) PreSign(ctx context.Context) (map[string]string, error) {
	...
	signedUrl, header, err := m.Signer.PresignHTTP(
		ctx,
		creds,
		req,
		signPayload,
...
func NewContextWithMetadata(
	...
	return sasl.WithMetadata(ctx, &sasl.Metadata{
		Host: host,
		Port: port,
	})
}
