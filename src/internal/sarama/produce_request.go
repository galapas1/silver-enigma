...
func updateBatchMetrics(
	...
	compressionRatioMetric metrics.Histogram,
	topicCompressionRatioMetric metrics.Histogram,
) int64 {
	if recordBatch.compressedRecords != nil {
		compressionRatio := int64(
			float64(
				...
				len(recordBatch.compressedRecords),
			) * 100,
		)
...
func (r *ProduceRequest) encode(pe packetEncoder) error {
	...
	if metricRegistry != nil {
		...
		compressionRatioMetric = getOrRegisterHistogram(
			"compression-ratio",
			metricRegistry,
		)
	}
	...
	for topic, partitions := range r.records {
		...
		if metricRegistry != nil {
			topicCompressionRatioMetric = getOrRegisterTopicHistogram(
				"compression-ratio",
				topic,
				metricRegistry,
			)
		...
		if topicRecordCount > 0 {
			getOrRegisterTopicMeter(
				"record-send-rate",
				topic,
				metricRegistry,
			).Mark(topicRecordCount)
			getOrRegisterTopicHistogram(
				"records-per-request",
				topic,
				metricRegistry,
			).Update(topicRecordCount)
			totalRecordCount += topicRecordCount
		}
	}
	if totalRecordCount > 0 {
		metrics.GetOrRegisterMeter("record-send-rate", metricRegistry).
			Mark(totalRecordCount)
