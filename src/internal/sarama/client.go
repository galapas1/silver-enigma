...
func NewClient(addrs []string, conf *Config) (Client, error) {
	...
	if strings.Contains(addrs[0], ".servicebus.windows.net") {
		if conf.Version.IsAtLeast(V1_1_0_0) || !conf.Version.IsAtLeast(V0_11_0_0) {
			Logger.Println(
				"Connecting to Azure Event Hubs, forcing version to V1_0_0_0 for compatibility",
			)
...
func (client *client) InitProducerID() (*InitProducerIDResponse, error) {
	...
	for broker := client.LeastLoadedBroker(); broker != nil; broker = client.LeastLoadedBroker() {
		...
		if err == nil {
			...
			Logger.Printf("Client got error from broker %s when issuing InitProducerID : %v\n", broker.Addr(), err)
			_ = broker.Close()
...
func (client *client) OfflineReplicas(
	topic string,
	partitionID int32,
) ([]int32, error) {
	if client.Closed() {
...
func (client *client) LeaderAndEpoch(
	topic string,
	partitionID int32,
) (*Broker, int32, error) {
	if client.Closed() {
...
func (client *client) RefreshCoordinator(consumerGroup string) error {
	...
	response, err := client.findCoordinator(
		consumerGroup,
		CoordinatorGroup,
		client.conf.Metadata.Retry.Max,
	)
	if err != nil {
		return err
	}
...
func (client *client) updateBroker(brokers []*Broker) {
	...
	for _, broker := range brokers {
		...
		if client.brokers[broker.ID()] == nil { // add new broker
			...
			DebugLogger.Printf(
				"client/brokers registered new broker #%d at %s",
				broker.ID(),
				broker.Addr(),
			)
	...
	for id, broker := range client.brokers {
		if _, exist := currentBroker[id]; !exist { // remove old broker
			...
			Logger.Printf(
				"client/broker remove invalid broker #%d with %s",
				broker.ID(),
				broker.Addr(),
			)
		}
...
func (client *client) registerBroker(broker *Broker) {
	if client.brokers == nil {
		Logger.Printf(
			"cannot register broker #%d at %s, client already closed",
			broker.ID(),
			broker.Addr(),
		)
		return
...
func (client *client) deregisterBroker(broker *Broker) {
	...
	if ok {
		Logger.Printf(
			"client/brokers deregistered broker #%d at %s",
			broker.ID(),
			broker.Addr(),
		)
		delete(client.brokers, broker.ID())
...
func (client *client) resurrectDeadBrokers() {
	...
	Logger.Printf(
		"client/brokers resurrecting %d dead seed brokers",
		len(client.deadSeeds),
	)
...
func (client *client) cachedMetadata(
	topic string,
	partitionID int32,
) *PartitionMetadata {
	client.lock.RLock()
...
func (client *client) cachedPartitions(
	topic string,
	partitionSet partitionType,
) []int32 {
	client.lock.RLock()
	defer client.lock.RUnlock()
...
func (client *client) setPartitionCache(
	topic string,
	partitionSet partitionType,
) []int32 {
	partitions := client.metadata[topic]
	...
	for _, partition := range partitions {
		if partitionSet == writablePartitions &&
			errors.Is(partition.Err, ErrLeaderNotAvailable) {
			continue
		}
...
func (client *client) cachedLeader(
	topic string,
	partitionID int32,
) (*Broker, int32, error) {
	client.lock.RLock()
...
func (client *client) tryRefreshMetadata(
	topics []string,
	...
	retry := func(err error) error {
		if attemptsRemaining > 0 {
			...
			if pastDeadline(backoff) {
				Logger.Println(
					"client/metadata skipping last retries as we would go past the metadata timeout",
				)
				return err
			}
			if backoff > 0 {
	...
	for ; broker != nil && !pastDeadline(0); broker = client.LeastLoadedBroker() {
		...
		if len(topics) > 0 {
			DebugLogger.Printf(
				"client/metadata fetching metadata for %v from broker %s\n",
				topics,
				broker.Addr(),
			)
		} else {
		...
		if err == nil {
			...
			Logger.Printf("client/metadata (1) got error from broker %s while fetching metadata: %v\n", broker.Addr(), err)
			_ = broker.Close()
			client.deregisterBroker(broker)
		} else {
			// some other error, remove that broker and try again
			Logger.Printf("client/metadata (2) got error from broker %s while fetching metadata: %v\n", broker.Addr(), err)
			brokerErrors = append(brokerErrors, err)
			_ = broker.Close()
			client.deregisterBroker(broker)
		}
	...
	if broker != nil {
		Logger.Printf(
			"client/metadata not fetching metadata from broker %s as we would go past the metadata timeout\n",
			broker.Addr(),
		)
		return retry(error)
	}
...
func (client *client) updateMetadata(
	...
	for _, topic := range data.Topics {
		...
		case ErrInvalidTopic,
			ErrTopicAuthorizationFailed: // don't retry, don't store partial results
			err = topic.Err
			continue
		...
		client.metadata[topic.Name] = make(
			map[int32]*PartitionMetadata,
		...
		partitionCache[allPartitions] = client.setPartitionCache(
			topic.Name,
			allPartitions,
		)
		partitionCache[writablePartitions] = client.setPartitionCache(
			topic.Name,
			writablePartitions,
		)
...
func (client *client) findCoordinator(
	...
	retry := func(err error) (*FindCoordinatorResponse, error) {
		if attemptsRemaining > 0 {
			...
			attemptsRemaining--
			Logger.Printf(
				"client/coordinator retrying after %dms... (%d attempts remaining)\n",
				backoff/time.Millisecond,
				attemptsRemaining,
			)
			time.Sleep(backoff)
			return client.findCoordinator(
				coordinatorKey,
				coordinatorType,
				attemptsRemaining,
			)
	...
	for broker := client.LeastLoadedBroker(); broker != nil; broker = client.LeastLoadedBroker() {
		DebugLogger.Printf(
			"client/coordinator requesting coordinator for %s from %s\n",
			coordinatorKey,
			broker.Addr(),
		)
		...
		if err != nil {
			Logger.Printf(
				"client/coordinator request to broker %s failed: %v\n",
				broker.Addr(),
				err,
			)
		...
		if errors.Is(response.Err, ErrNoError) {
			DebugLogger.Printf(
				"client/coordinator coordinator for %s is #%d (%s)\n",
				coordinatorKey,
				response.Coordinator.ID(),
				response.Coordinator.Addr(),
			)
			return response, nil
	...
	Logger.Println(
		"client/coordinator no available broker to send consumer metadata request to",
	)
	client.resurrectDeadBrokers()
