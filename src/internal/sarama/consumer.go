...
func (ce ConsumerError) Error() string {
	return fmt.Sprintf(
		"kafka: error while consuming %s/%d: %s",
		ce.Topic,
		ce.Partition,
		ce.Err,
	)
...
var errTimedOut = errors.New(
	"timed out feeding messages to the user",
) // not user-facing
...
func (child *partitionConsumer) chooseStartingOffset(offset int64) error {
	newestOffset, err := child.consumer.client.GetOffset(
		child.topic,
		child.partition,
		OffsetNewest,
	)
	...
	oldestOffset, err := child.consumer.client.GetOffset(
		child.topic,
		child.partition,
		OffsetOldest,
	)
...
func (child *partitionConsumer) parseMessages(
	msgSet *MessageSet,
...
func (child *partitionConsumer) parseRecords(
	batch *RecordBatch,
) ([]*ConsumerMessage, error) {
...
func (child *partitionConsumer) parseResponse(
	response *FetchResponse,
) ([]*ConsumerMessage, error) {
	var consumerBatchSizeMetric metrics.Histogram
	if child.consumer != nil && child.consumer.metricRegistry != nil {
		consumerBatchSizeMetric = getOrRegisterHistogram(
			"consumer-batch-size",
			child.consumer.metricRegistry,
		)
	}
	...
	if nRecs == 0 {
		...
		if partialTrailingMessage {
			if child.conf.Consumer.Fetch.Max > 0 &&
...
func (bc *brokerConsumer) subscriptionConsumer() {
	for newSubscriptions := range bc.newSubscriptions {
		...
		if err != nil {
			Logger.Printf(
				"consumer/broker/%d disconnecting due to error processing FetchRequest: %s\n",
				bc.broker.ID(),
				err,
			)
			bc.abort(err)
...
func (bc *brokerConsumer) updateSubscriptions(newSubscriptions []*partitionConsumer) {
	for _, child := range newSubscriptions {
		...
		Logger.Printf(
			"consumer/broker/%d added subscription to %s/%d\n",
			bc.broker.ID(),
			child.topic,
			child.partition,
		)
	...
	for child := range bc.subscriptions {
		...
		case <-child.dying:
			Logger.Printf(
				"consumer/broker/%d closed dead subscription to %s/%d\n",
				bc.broker.ID(),
				child.topic,
				child.partition,
			)
...
func (bc *brokerConsumer) handleResponses() {
	for child := range bc.subscriptions {
		...
		if errors.Is(result, errTimedOut) {
			Logger.Printf(
				"consumer/broker/%d abandoned subscription to %s/%d because consuming was taking too long\n",
				bc.broker.ID(),
				child.topic,
				child.partition,
			)
