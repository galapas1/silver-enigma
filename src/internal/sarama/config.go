...
func NewConfig() *Config {
	...
	c.Net.SASL.Version = SASLHandshakeV0 // REVIEW: jrb
	...
	c.Version = V0_10_0_0 //  DefaultVersion // REVIEW: jrb
...
func (c *Config) Validate() error {
	...
	if !c.Net.SASL.Enable {
		if c.Net.SASL.User != "" {
			Logger.Println(
				"Net.SASL is disabled but a non-empty username was provided.",
			)
		}
		if c.Net.SASL.Password != "" {
			Logger.Println(
				"Net.SASL is disabled but a non-empty password was provided.",
			)
		}
	}
	if c.Producer.RequiredAcks > 1 {
		Logger.Println(
			"Producer.RequiredAcks > 1 is deprecated and will raise an exception with kafka >= 0.8.2.0.",
		)
	}
	if c.Producer.MaxMessageBytes >= int(MaxRequestSize) {
		Logger.Println(
			"Producer.MaxMessageBytes must be smaller than MaxRequestSize; it will be ignored.",
		)
	}
	if c.Producer.Flush.Bytes >= int(MaxRequestSize) {
		Logger.Println(
			"Producer.Flush.Bytes must be smaller than MaxRequestSize; it will be ignored.",
		)
	}
	if (c.Producer.Flush.Bytes > 0 || c.Producer.Flush.Messages > 0) &&
		c.Producer.Flush.Frequency == 0 {
		Logger.Println(
			"Producer.Flush: Bytes or Messages are set, but Frequency is not; messages may not get flushed.",
		)
	}
	if c.Producer.Timeout%time.Millisecond != 0 {
		Logger.Println(
			"Producer.Timeout only supports millisecond resolution; nanoseconds will be truncated.",
		)
	}
	...
	if c.Consumer.MaxWaitTime%time.Millisecond != 0 {
		Logger.Println(
			"Consumer.MaxWaitTime only supports millisecond precision; nanoseconds will be truncated.",
		)
	}
	if c.Consumer.Offsets.Retention%time.Millisecond != 0 {
		Logger.Println(
			"Consumer.Offsets.Retention only supports millisecond precision; nanoseconds will be truncated.",
		)
	}
	if c.Consumer.Group.Session.Timeout%time.Millisecond != 0 {
		Logger.Println(
			"Consumer.Group.Session.Timeout only supports millisecond precision; nanoseconds will be truncated.",
		)
	}
	if c.Consumer.Group.Heartbeat.Interval%time.Millisecond != 0 {
		Logger.Println(
			"Consumer.Group.Heartbeat.Interval only supports millisecond precision; nanoseconds will be truncated.",
		)
	}
	if c.Consumer.Group.Rebalance.Timeout%time.Millisecond != 0 {
		Logger.Println(
			"Consumer.Group.Rebalance.Timeout only supports millisecond precision; nanoseconds will be truncated.",
		)
	}
	...
	case c.Net.SASL.Enable:
		...
		default:
			msg := fmt.Sprintf(
				"The SASL mechanism configuration is invalid. Possible values are `%s`, `%s`, `%s`, `%s`, `%s` and `%s`",
				SASLTypeAWSMSKIAM,
				SASLTypeOAuth,
				SASLTypePlaintext,
				SASLTypeSCRAMSHA256,
	...
	if c.Producer.Compression == CompressionGZIP {
		if c.Producer.CompressionLevel != CompressionLevelDefault {
			if _, err := gzip.NewWriterLevel(io.Discard, c.Producer.CompressionLevel); err != nil {
				return ConfigurationError(
					fmt.Sprintf(
						"gzip compression does not work with level %d: %v",
						c.Producer.CompressionLevel,
						err,
					),
				)
			}
	...
	if c.Producer.Transaction.ID != "" && !c.Producer.Idempotent {
		return ConfigurationError(
			"Transactional producer requires Idempotent to be true",
		)
	}
	...
	case c.Consumer.Offsets.Initial != OffsetOldest && c.Consumer.Offsets.Initial != OffsetNewest:
		return ConfigurationError(
			"Consumer.Offsets.Initial must be OffsetOldest or OffsetNewest",
		)
	...
	if c.Consumer.Offsets.CommitInterval != 0 {
		Logger.Println(
			"Deprecation warning: Consumer.Offsets.CommitInterval exists for historical compatibility" +
	...
	if c.Consumer.Group.Rebalance.Strategy != nil {
		Logger.Println(
			"Deprecation warning: Consumer.Group.Rebalance.Strategy exists for historical compatibility" +
	...
	case c.Consumer.Group.Heartbeat.Interval >= c.Consumer.Group.Session.Timeout:
		return ConfigurationError(
			"Consumer.Group.Heartbeat.Interval must be < Consumer.Group.Session.Timeout",
		)
	case c.Consumer.Group.Rebalance.Strategy == nil && len(c.Consumer.Group.Rebalance.GroupStrategies) == 0:
		return ConfigurationError(
			"Consumer.Group.Rebalance.GroupStrategies or Consumer.Group.Rebalance.Strategy must not be empty",
		)
	...
	for _, strategy := range c.Consumer.Group.Rebalance.GroupStrategies {
		if strategy == nil {
			return ConfigurationError(
				"elements in Consumer.Group.Rebalance.Strategies must not be empty",
			)
		}
	}
...
func validateGroupInstanceId(id string) error {
	...
	if len(id) > MAX_GROUP_INSTANCE_ID_LENGTH {
		return ConfigurationError(
			fmt.Sprintf(
				`Group instance id cannot be longer than %v, characters: %s`,
				MAX_GROUP_INSTANCE_ID_LENGTH,
				id,
			),
		)
	}
	if !GROUP_INSTANCE_ID_REGEXP.MatchString(id) {
		return ConfigurationError(
			fmt.Sprintf(
				`Group instance id %s is illegal, it contains a character other than, '.', '_' and '-'`,
				id,
			),
		)
	}
