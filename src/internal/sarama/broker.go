...
import (
	...
	awsiam "optm.com/ninja-panda/src/internal/awsiam"
)
...
func (b *Broker) Open(conf *Config) error {
	...
	usingApiVersionsRequests := conf.Version.IsAtLeast(V2_4_0_0) &&
		conf.ApiVersionsRequest
	...
	go withRecover(func() {
		defer func() {
			...
			if usingApiVersionsRequests {
				...
				if err != nil {
					Logger.Printf(
						"Error while sending ApiVersionsRequest to broker %s: %s\n",
						b.addr,
						err,
					)
				}
		...
		b.incomingByteRate = metrics.GetOrRegisterMeter(
			"incoming-byte-rate",
			b.metricRegistry,
		)
		b.requestRate = metrics.GetOrRegisterMeter("request-rate", b.metricRegistry)
		b.fetchRate = metrics.GetOrRegisterMeter(
			"consumer-fetch-rate",
			b.metricRegistry,
		)
		b.requestSize = getOrRegisterHistogram("request-size", b.metricRegistry)
		b.requestLatency = getOrRegisterHistogram(
			"request-latency-in-ms",
			b.metricRegistry,
		)
		b.outgoingByteRate = metrics.GetOrRegisterMeter(
			"outgoing-byte-rate",
			b.metricRegistry,
		)
		...
		if conf.Net.SASL.Mechanism == SASLTypeOAuth &&
		...
		if b.id >= 0 {
			DebugLogger.Printf(
				"Connected to broker at %s (registered as #%d)\n",
				b.addr,
				b.id,
			)
		} else {
...
func (b *Broker) GetConsumerMetadata(
	request *ConsumerMetadataRequest,
) (*ConsumerMetadataResponse, error) {
...
func (b *Broker) CreateTopics(
	request *CreateTopicsRequest,
) (*CreateTopicsResponse, error) {
...
func (b *Broker) DeleteTopics(
	request *DeleteTopicsRequest,
) (*DeleteTopicsResponse, error) {
...
func (b *Broker) CreatePartitions(
...
func (b *Broker) AlterPartitionReassignments(
...
func (b *Broker) ListPartitionReassignments(
...
func (b *Broker) DescribeAcls(
	request *DescribeAclsRequest,
) (*DescribeAclsResponse, error) {
...
func (b *Broker) InitProducerID(
	request *InitProducerIDRequest,
) (*InitProducerIDResponse, error) {
...
// a response or error
func (b *Broker) AddPartitionsToTxn(
...
func (b *Broker) DeleteOffsets(
	request *DeleteOffsetsRequest,
) (*DeleteOffsetsResponse, error) {
...
func (b *Broker) DescribeLogDirs(
	request *DescribeLogDirsRequest,
) (*DescribeLogDirsResponse, error) {
...
func (b *Broker) DescribeUserScramCredentials(
	req *DescribeUserScramCredentialsRequest,
) (*DescribeUserScramCredentialsResponse, error) {
...
func (b *Broker) AlterUserScramCredentials(
	req *AlterUserScramCredentialsRequest,
) (*AlterUserScramCredentialsResponse, error) {
...
func (b *Broker) DescribeClientQuotas(
	request *DescribeClientQuotasRequest,
) (*DescribeClientQuotasResponse, error) {
...
func (b *Broker) AlterClientQuotas(
	request *AlterClientQuotasRequest,
) (*AlterClientQuotasResponse, error) {
...
func (b *Broker) sendWithPromise(rb protocolBody, promise *responsePromise) error {
	...
	if b.clientSessionReauthenticationTimeMs > 0 &&
		currentUnixMilli() > b.clientSessionReauthenticationTimeMs {
	...
	for response := range b.responses {
		...
		if decodedHeader.correlationID != response.correlationID {
			...
			dead = PacketDecodingError{
				fmt.Sprintf(
					"correlation ID didn't match, wanted %d, got %d",
		...
		b.updateIncomingCommunicationMetrics(
			bytesReadHeader+bytesReadBody,
			requestLatency,
		)
		if err != nil {
			dead = err
...
func (b *Broker) authenticateViaSASLv1() error {
	...
	if b.conf.Net.SASL.Handshake {
		handshakeRequest := &SaslHandshakeRequest{
			Mechanism: string(b.conf.Net.SASL.Mechanism),
			Version:   b.conf.Net.SASL.Version,
		}
		...
		handshakeErr = handleResponsePromise(
			handshakeRequest,
			handshakeResponse,
			prom,
			metricRegistry,
	...
	authSendReceiver := func(authBytes []byte) (*SaslAuthenticateResponse, error) {
		...
		if !errors.Is(authenticateResponse.Err, ErrNoError) {
			...
			if authenticateResponse.ErrorMessage != nil {
				err = Wrap(
					authenticateResponse.Err,
					errors.New(*authenticateResponse.ErrorMessage),
				)
			}
			return nil, err
		}
	...
	case SASLTypeSCRAMSHA256, SASLTypeSCRAMSHA512:
		return b.sendAndReceiveSASLSCRAMv1(
			authSendReceiver,
			b.conf.Net.SASL.SCRAMClientGeneratorFunc(),
		)
	default:
...
func (b *Broker) sendAndReceiveSASLHandshake(
	...
	DebugLogger.Print(
		"Completed pre-auth SASL handshake. Available mechanisms: ",
		res.EnabledMechanisms,
	)
	return nil
}
...
func (b *Broker) sendAndReceiveSASLPlainAuthV0() error {
	...
	if b.conf.Net.SASL.Handshake {
		handshakeErr := b.sendAndReceiveSASLHandshake(
			SASLTypePlaintext,
			b.conf.Net.SASL.Version,
		)
		if handshakeErr != nil {
			Logger.Printf("Error while performing SASL handshake %s\n", b.addr)
			return handshakeErr
	...
	length := len(
		b.conf.Net.SASL.AuthIdentity,
	) + 1 + len(
		b.conf.Net.SASL.User,
	) + 1 + len(
		b.conf.Net.SASL.Password,
	)
	authBytes := make([]byte, length+4) // 4 byte length header + auth data
	binary.BigEndian.PutUint32(authBytes, uint32(length))
	copy(
		authBytes[4:],
		b.conf.Net.SASL.AuthIdentity+"\x00"+b.conf.Net.SASL.User+"\x00"+b.conf.Net.SASL.Password,
	)
	...
	if err != nil {
		b.addRequestInFlightMetrics(-1)
		Logger.Printf(
			"Failed to write SASL auth header to broker %s: %s\n",
			b.addr,
			err.Error(),
		)
		return err
	...
	if err != nil {
		Logger.Printf(
			"Failed to read response while authenticating with SASL to broker %s: %s\n",
			b.addr,
			err.Error(),
		)
		return err
	...
	DebugLogger.Printf(
		"SASL authentication successful with broker %s:%v - %v\n",
		b.addr,
		n,
		header,
	)
...
func (b *Broker) computeSaslSessionLifetime(res *SaslAuthenticateResponse) {
	if res.SessionLifetimeMs > 0 {
		...
		DebugLogger.Printf(
			"Session expiration in %d ms and session re-authentication on or after %d ms",
...
func (b *Broker) updateIncomingCommunicationMetrics(
	bytes int,
...
func (b *Broker) updateProtocolMetrics(rb protocolBody) {
	...
	if protocolRequestsRate == nil {
		protocolRequestsRate = metrics.GetOrRegisterMeter(
			fmt.Sprintf("protocol-requests-rate-%d", rb.key()),
			b.metricRegistry,
		)
	...
	if b.brokerProtocolRequestsRate != nil {
		...
		if brokerProtocolRequestsRate == nil {
			brokerProtocolRequestsRate = b.registerMeter(
				fmt.Sprintf("protocol-requests-rate-%d", rb.key()),
			)
...
func (b *Broker) sendAndReceiveSASLIAM() error {
	Logger.Println("** sendAndReceiveSASLIAM **")
...
func getIAMPayload(addr, useragent string, cfg AWSMSKIAMConfig) ([]byte, error) {
	DebugLogger.Printf("getIAMPayload Region (%s) Addr (%s)\n", cfg.Region, addr)
	...
	header, err := m.PreSign(
		awsiam.NewContextWithMetadata(context.Background(), host, port),
	)
	if err != nil {
		return nil, err
	}
	...
	for key, vals := range header {
		DebugLogger.Printf("getIAMPayload %s = %s", key, vals[0])
		payload[strings.ToLower(key)] = vals //[0]
	}

	DebugLogger.Printf("req.URL.Query(): %v\n", req.URL.Query())
...
func (b *Broker) sendSaslAuthenticateRequest(
	correlationID int32,
	msg []byte,
) (int, error) {
	// rb := &SaslAuthenticateRequest{msg} // REVIEW: jrb
...
func (b *Broker) receiveSaslAuthenticateResponse(correlationID int32) ([]byte, error) {
	buf := make([]byte, 8) // REVIEW: response.length.size + correlationID.size
	_, err := b.readFull(buf)
	if err != nil {
		Logger.Printf(
			"receiveSaslAuthenticateResponse: failed to read full %s: %w\n",
			b.addr,
			err,
		)
		return nil, err
	...
	header := saslResponseHeader{}
	err = decode(buf, &header, nil)
	if err != nil {
		Logger.Printf(
			"receiveSaslAuthenticateResponse: failed to decode %s: %w\n",
			b.addr,
			err,
		)
		return nil, err
	...
	buf = make([]byte, header.length-4) // REVIEW: correlationID.Size
	_, err = b.readFull(buf)
	if err != nil {
		Logger.Printf(
			"receiveSaslAuthenticateResponse: failed to read buf %s: %w\n",
			b.addr,
			err,
		)
		return nil, err
	...
	if err := versionedDecode(buf, res, 0, nil); err != nil { // decode, grasp at straw
		Logger.Printf(
			"receiveSaslAuthenticateResponse: decode error %s: %w\n",
			b.addr,
			err,
		)
		return nil, err
	...
	if res.Err != ErrNoError {
		Logger.Printf(
			"receiveSaslAuthenticateResponse: error in response %s: %w\n",
			b.addr,
			res.Err,
		)
		return nil, res.Err
