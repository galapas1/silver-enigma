...
var ErrClosedConsumerGroup = errors.New(
...
func (c *consumerGroup) Consume(
	...
	sess, err := c.newSession(
		...
		topics,
		handler,
		c.config.Consumer.Group.Rebalance.Retry.Max,
	)
...
func (c *consumerGroup) retryNewSession(
	ctx context.Context,
	topics []string,
	handler ConsumerGroupHandler,
	retries int,
	refreshCoordinator bool,
...
func (c *consumerGroup) newSession(
	ctx context.Context,
	topics []string,
	handler ConsumerGroupHandler,
	retries int,
	...
	if metricRegistry != nil {
		consumerGroupJoinTotal = metrics.GetOrRegisterCounter(
			fmt.Sprintf("consumer-group-join-total-%s", c.groupID),
			metricRegistry,
		)
		consumerGroupJoinFailed = metrics.GetOrRegisterCounter(
			fmt.Sprintf("consumer-group-join-failed-%s", c.groupID),
			metricRegistry,
		)
		consumerGroupSyncTotal = metrics.GetOrRegisterCounter(
			fmt.Sprintf("consumer-group-sync-total-%s", c.groupID),
			metricRegistry,
		)
		consumerGroupSyncFailed = metrics.GetOrRegisterCounter(
			fmt.Sprintf("consumer-group-sync-failed-%s", c.groupID),
			metricRegistry,
		)
	}
	...
	case ErrMemberIdRequired:
		...
		// group
		c.memberID = join.MemberId
		return c.retryNewSession(
			ctx,
			topics,
			handler,
			retries+1, /*keep retry time*/
			false,
		)
	case ErrFencedInstancedId:
		if c.groupInstanceId != nil {
			Logger.Printf(
				"JoinGroup failed: group instance id %s has been fenced\n",
				*c.groupInstanceId,
			)
		}
		return nil, join.Err
	...
	if strategy = c.config.Consumer.Group.Rebalance.Strategy; strategy == nil {
		...
		if !ok {
			...
			return nil, fmt.Errorf(
				"unable to find selected strategy: %s",
				join.GroupProtocol,
			)
		}
	}
	...
	if join.LeaderId == join.MemberId {
		...
		allSubscribedTopicPartitions, allSubscribedTopics, plan, err = c.balance(
			strategy,
			members,
		)
		if err != nil {
	...
	// Sync consumer group
	syncGroupResponse, err := c.syncGroupRequest(
		coordinator,
		members,
		plan,
		join.GenerationId,
	...
	session, err := newConsumerGroupSession(
		ctx,
		c,
		claims,
		join.MemberId,
	...
	if join.LeaderId == join.MemberId {
		go c.loopCheckPartitionNumbers(
			allSubscribedTopicPartitions,
			allSubscribedTopics,
			session,
		)
	}
...
func (c *consumerGroup) joinGroupRequest(
	coordinator *Broker,
	topics []string,
	...
	req := &JoinGroupRequest{
		...
		SessionTimeout: int32(
			c.config.Consumer.Group.Session.Timeout / time.Millisecond,
		),
		ProtocolType: "consumer",
	}
	if c.config.Version.IsAtLeast(V0_10_1_0) {
		req.Version = 1
		req.RebalanceTimeout = int32(
			c.config.Consumer.Group.Rebalance.Timeout / time.Millisecond,
		)
	}
...
func (c *consumerGroup) findStrategy(
	name string,
	groupStrategies []BalanceStrategy,
) (BalanceStrategy, bool) {
...
func (c *consumerGroup) heartbeatRequest(
	coordinator *Broker,
	memberID string,
	generationID int32,
...
func (c *consumerGroup) balance(
	strategy BalanceStrategy,
	members map[string]ConsumerGroupMemberMetadata,
) (map[string][]int32, []string, BalanceStrategyPlan, error) {
...
func (c *consumerGroup) loopCheckPartitionNumbers(
...
func (c *consumerGroup) topicToPartitionNumbers(
	topics []string,
...
func newConsumerGroupSession(
	...
	// init offset manager
	offsets, err := newOffsetManagerFromClient(
		parent.groupID,
		memberID,
		generationID,
		parent.client,
...
func (s *consumerGroupSession) MarkOffset(
	topic string,
	partition int32,
	offset int64,
	metadata string,
...
func (s *consumerGroupSession) ResetOffset(
	topic string,
	partition int32,
	offset int64,
	metadata string,
