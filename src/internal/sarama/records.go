...
func (r *Records) setTypeFromFields() (bool, error) {
	...
	if r.MsgSet != nil && r.RecordBatch != nil {
		return false, fmt.Errorf(
			"both MsgSet and RecordBatch are set, but record type is unknown",
		)
	}
...
func (r *Records) getControlRecord() (ControlRecord, error) {
	if r.RecordBatch == nil || len(r.RecordBatch.Records) == 0 {
		return ControlRecord{}, fmt.Errorf(
			"cannot get control record, record batch is empty",
		)
	}

	firstRecord := r.RecordBatch.Records[0]
	controlRecord := ControlRecord{}
	err := controlRecord.decode(
		&realDecoder{raw: firstRecord.Key},
		&realDecoder{raw: firstRecord.Value},
	)
	if err != nil {
