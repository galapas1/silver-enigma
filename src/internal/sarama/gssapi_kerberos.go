...
func (krbAuth *GSSAPIKerberosAuth) writePackage(
	broker *Broker,
	payload []byte,
) (int, error) {
...
func (krbAuth *GSSAPIKerberosAuth) initSecContext(
	...
	case GSS_API_VERIFY:
		...
		// Validate response.
		isValid, err := wrapTokenReq.Verify(
			krbAuth.encKey,
		...
		wrapTokenResponse, err := gssapi.NewInitiatorWrapToken(
			wrapTokenReq.Payload,
			krbAuth.encKey,
		)
		if err != nil {
			return nil, err
...
func (krbAuth *GSSAPIKerberosAuth) Authorize(broker *Broker) error {
	...
	for {
		...
		if err != nil {
			Logger.Printf(
				"Error while performing GSSAPI Kerberos Authentication: %s\n",
				err,
			)
			return err
		}
