...
func (t *transactionManager) transitionTo(
	target ProducerTxnStatusFlag,
	err error,
) error {
	t.statusLock.Lock()
	...
	DebugLogger.Printf(
		"txnmgr/transition [%s] transition from %s to %s\n",
		t.transactionalID,
		t.status,
		target,
	)
...
func (t *transactionManager) getAndIncrementSequenceNumber(
	topic string,
	partition int32,
) (int32, int16) {
...
func (t *transactionManager) computeBackoff(attemptsRemaining int) time.Duration {
	if t.client.Config().Producer.Transaction.Retry.BackoffFunc != nil {
		...
		return t.client.Config().Producer.Transaction.Retry.BackoffFunc(
			retries,
			maxRetries,
		)
	}
...
func (t *transactionManager) addOffsetsToTxn(
	offsetsToAdd map[string][]*PartitionOffsetMetadata,
	groupId string,
) error {
	t.mutex.Lock()
	defer t.mutex.Unlock()
...
func (t *transactionManager) publishOffsetsToTxn(
	offsets topicPartitionOffsets,
	groupId string,
) (topicPartitionOffsets, error) {
	...
	exec := func(run func() (bool, error), err error) error {
		for attemptsRemaining >= 0 {
			...
			Logger.Printf(
				"txnmgr/add-offset-to-txn [%s] retrying after %dms... (%d attempts remaining) (%s)\n",
				t.transactionalID,
	...
	lastError := exec(func() (bool, error) {
		...
		if response.Err == ErrNoError {
			DebugLogger.Printf(
				"txnmgr/add-offset-to-txn [%s] successful add-offset-to-txn with group %s %+v\n",
				t.transactionalID,
				groupId,
				response,
			)
	...
	execTxnOffsetCommit := func(run func() (topicPartitionOffsets, bool, error), err error) (topicPartitionOffsets, error) {
		...
		for attemptsRemaining >= 0 {
			...
			backoff := t.computeBackoff(attemptsRemaining)
			Logger.Printf(
				"txnmgr/txn-offset-commit [%s] retrying after %dms... (%d attempts remaining) (%s)\n",
				t.transactionalID,
				backoff/time.Millisecond,
				attemptsRemaining,
				err,
	...
	return execTxnOffsetCommit(func() (topicPartitionOffsets, bool, error) {
		...
		for topic, partitionErrors := range responses.Topics {
			for _, partitionError := range partitionErrors {
				...
				case ErrFencedInstancedId:
					fallthrough
				case ErrGroupAuthorizationFailed:
					return resultOffsets, false, t.transitionTo(
						ProducerTxnFlagInError|ProducerTxnFlagAbortableError,
						partitionError.Err,
					)
				default:
		...
		if len(resultOffsets) == 0 {
			DebugLogger.Printf(
				"txnmgr/txn-offset-commit [%s] successful txn-offset-commit with group %s %+v\n",
				t.transactionalID,
				groupId,
			)
...
func (t *transactionManager) initProducerId() (int64, int16, error) {
	...
	if isEpochBump {
		...
		DebugLogger.Printf(
			"txnmgr/init-producer-id [%s] invoking InitProducerId for the first time in order to acquire a producer ID\n",
			t.transactionalID,
		)
	} else {
	...
	exec := func(run func() (int64, int16, bool, error), err error) (int64, int16, error) {
		...
		for attemptsRemaining >= 0 {
			...
			backoff := t.computeBackoff(attemptsRemaining)
			Logger.Printf(
				"txnmgr/init-producer-id [%s] retrying after %dms... (%d attempts remaining) (%s)\n",
				t.transactionalID,
				backoff/time.Millisecond,
				attemptsRemaining,
				err,
	...
	return exec(func() (int64, int16, bool, error) {
		...
		if response.Err == ErrNoError {
			...
			DebugLogger.Printf(
				"txnmgr/init-producer-id [%s] successful init producer id %+v\n",
				t.transactionalID,
				response,
			)
		...
		// Fatal errors
		default:
			return -1, -1, false, t.transitionTo(
				ProducerTxnFlagInError|ProducerTxnFlagFatalError,
				response.Err,
			)
		}
...
func (t *transactionManager) endTxn(commit bool) error {
	...
	exec := func(run func() (bool, error), err error) error {
		for attemptsRemaining >= 0 {
			...
			backoff := t.computeBackoff(attemptsRemaining)
			Logger.Printf(
				"txnmgr/endtxn [%s] retrying after %dms... (%d attempts remaining) (%s)\n",
				t.transactionalID,
				backoff/time.Millisecond,
				attemptsRemaining,
				err,
...
func (t *transactionManager) maybeAddPartitionToCurrentTxn(
	topic string,
	partition int32,
) {
...
func (t *transactionManager) publishTxnPartitions() error {
	...
	exec := func(run func() (bool, error), err error) error {
		for attemptsRemaining >= 0 {
			...
			Logger.Printf(
				"txnmgr/add-partition-to-txn retrying after %dms... (%d attempts remaining) (%s)\n",
				backoff/time.Millisecond,
				attemptsRemaining,
				err,
			)
	...
	return exec(func() (bool, error) {
		...
		if len(t.pendingPartitionsInCurrentTxn) == 0 {
			DebugLogger.Printf(
				"txnmgr/add-partition-to-txn [%s] successful to add partitions txn %+v\n",
				t.transactionalID,
				addPartResponse,
			)
			return false, nil
...
func newTransactionManager(conf *Config, client Client) (*transactionManager, error) {
	...
	if conf.Producer.Idempotent {
		...
		Logger.Printf(
			"txnmgr/init-producer-id [%s] obtained a ProducerId: %d and ProducerEpoch: %d\n",
			txnmgr.transactionalID,
			txnmgr.producerID,
			txnmgr.producerEpoch,
		)
	}
