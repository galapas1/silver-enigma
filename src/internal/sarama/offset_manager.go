...
func (om *offsetManager) ManagePartition(
	topic string,
	partition int32,
) (PartitionOffsetManager, error) {
...
func (om *offsetManager) fetchInitialOffset(
	topic string,
	partition int32,
	retries int,
) (int64, int32, string, error) {
...
func (om *offsetManager) handleResponse(
	broker *Broker,
	...
	for _, topicManagers := range om.poms {
		for _, pom := range topicManagers {
			if req.blocks[pom.topic] == nil ||
				req.blocks[pom.topic][pom.partition] == nil {
				continue
			}
...
func (om *offsetManager) newPartitionOffsetManager(
	topic string,
	partition int32,
) (*partitionOffsetManager, error) {
	offset, leaderEpoch, metadata, err := om.fetchInitialOffset(
		topic,
		partition,
		om.conf.Metadata.Retry.Max,
	)
	if err != nil {
		return nil, err
	}
