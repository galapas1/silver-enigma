...
func (pe ProducerError) Error() string {
	return fmt.Sprintf(
		"kafka: Failed to produce message to topic %s: %s",
		pe.Msg.Topic,
		pe.Err,
	)
}
...
func (p *asyncProducer) AddMessageToTxn(
	msg *ConsumerMessage,
	groupId string,
	metadata *string,
) error {
...
func (p *asyncProducer) AddOffsetsToTxn(
	offsets map[string][]*PartitionOffsetMetadata,
	groupId string,
) error {
	p.txLock.Lock()
	...
	if !p.IsTransactional() {
		DebugLogger.Printf(
			"producer/txnmgr [%s] attempt to call AddOffsetsToTxn on a non-transactional producer\n",
	...
	DebugLogger.Printf(
		"producer/txnmgr [%s] add offsets to transaction\n",
		p.txnmgr.transactionalID,
	)
...
func (p *asyncProducer) BeginTxn() error {
	...
	if !p.IsTransactional() {
		DebugLogger.Println(
			"producer/txnmgr attempt to call BeginTxn on a non-transactional producer",
		)
		return ErrNonTransactedProducer
	}
...
func (p *asyncProducer) CommitTxn() error {
	...
	if !p.IsTransactional() {
		DebugLogger.Printf(
			"producer/txnmgr [%s] attempt to call CommitTxn on a non-transactional producer\n",
			p.txnmgr.transactionalID,
		)
		return ErrNonTransactedProducer
	}
	...
	DebugLogger.Printf(
		"producer/txnmgr [%s] committing transaction\n",
		p.txnmgr.transactionalID,
	)
	...
	DebugLogger.Printf(
		"producer/txnmgr [%s] transaction committed\n",
		p.txnmgr.transactionalID,
	)
	return nil
}
...
func (p *asyncProducer) AbortTxn() error {
	...
	if !p.IsTransactional() {
		DebugLogger.Printf(
			"producer/txnmgr [%s] attempt to call AbortTxn on a non-transactional producer\n",
			p.txnmgr.transactionalID,
		)
		return ErrNonTransactedProducer
	}
	DebugLogger.Printf(
		"producer/txnmgr [%s] aborting transaction\n",
		p.txnmgr.transactionalID,
	)
	...
	DebugLogger.Printf(
		"producer/txnmgr [%s] transaction aborted\n",
		p.txnmgr.transactionalID,
	)
	return nil
}
...
func (p *asyncProducer) dispatcher() {
	...
	for msg := range p.input {
		...
		if msg.retries == 0 {
			...
			if p.IsTransactional() &&
				p.txnmgr.currentTxnStatus()&ProducerTxnFlagInTransaction == 0 {
				Logger.Printf(
					"attempt to send message when transaction is not started or is in ending state, got %d, expect %d\n",
					p.txnmgr.currentTxnStatus(),
		...
		if size > p.conf.Producer.MaxMessageBytes {
			p.returnError(
				...
				ConfigurationError(
					fmt.Sprintf(
						"Attempt to produce message larger than configured Producer.MaxMessageBytes: %d > %d",
						size,
...
func (pp *partitionProducer) updateLeaderIfBrokerProducerIsNil(
	msg *ProducerMessage,
) error {
	if pp.brokerProducer == nil {
		...
		Logger.Printf(
			"producer/leader/%s/%d selected broker %d\n",
			pp.topic,
			pp.partition,
			pp.leader.ID(),
		)
	...
	for msg := range pp.input {
		if pp.brokerProducer != nil && pp.brokerProducer.abandoned != nil {
			...
			case <-pp.brokerProducer.abandoned:
				...
				Logger.Printf(
					"producer/leader/%s/%d abandoning broker %d\n",
					pp.topic,
					pp.partition,
					pp.leader.ID(),
				)
		...
		if pp.parent.conf.Producer.Idempotent && msg.retries == 0 && msg.flags == 0 {
			msg.sequenceNumber, msg.producerEpoch = pp.parent.txnmgr.getAndIncrementSequenceNumber(
				msg.Topic,
				msg.Partition,
			)
...
func (pp *partitionProducer) newHighWatermark(hwm int) {
	Logger.Printf(
		"producer/leader/%s/%d state change to [retrying-%d]\n",
		pp.topic,
		pp.partition,
		hwm,
	)
	...
	pp.parent.inFlight.Add(
		1,
	) // we're generating a fin message; track it so we don't shut down while it's still inflight
...
func (pp *partitionProducer) flushRetryBuffers() {
	Logger.Printf(
		"producer/leader/%s/%d state change to [flushing-%d]\n",
		pp.topic,
		pp.partition,
		pp.highWatermark,
	)
		...
		case msg, ok := <-bp.input:
			...
			if reason := bp.needsRetry(msg); reason != nil {
				...
				if bp.closing == nil && msg.flags&fin == fin {
					...
					Logger.Printf(
						"producer/broker/%d state change to [closed] on %s/%d\n",
						bp.broker.ID(),
						msg.Topic,
						msg.Partition,
					)
			...
			if msg.flags&fin == fin {
				...
				DebugLogger.Printf(
					"producer/broker/%d state change to [dying-%d] on %s/%d\n",
					bp.broker.ID(),
					msg.retries,
					msg.Topic,
					msg.Partition,
			...
			if bp.buffer.wouldOverflow(msg) {
				Logger.Printf(
					"producer/broker/%d maximum request accumulated, waiting for space\n",
					bp.broker.ID(),
				)
			...
			if bp.parent.txnmgr.producerID != noProducerID &&
				bp.buffer.producerEpoch != msg.producerEpoch {
				// The epoch was reset, need to roll the buffer over
				Logger.Printf(
					"producer/broker/%d detected epoch rollover, waiting for new buffer\n",
					bp.broker.ID(),
				)
	...
	if len(retryTopics) > 0 {
		...
		sent.eachPartition(func(topic string, partition int32, pSet *partitionSet) {
			...
			case ErrInvalidMessage,
				...
				ErrNotEnoughReplicasAfterAppend:
				Logger.Printf(
					"producer/broker/%d state change to [retrying] on %s/%d because %v\n",
					bp.broker.ID(),
					topic,
					partition,
					block.Err,
				...
				bp.parent.retryMessages(
					bp.buffer.dropPartition(topic, partition),
					block.Err,
				)
			}
		})
...
func (p *asyncProducer) retryBatch(
	topic string,
	partition int32,
	pSet *partitionSet,
	kerr KError,
	...
	if err != nil {
		Logger.Printf(
			"Failed retrying batch for %v-%d because of %v while looking up for new leader\n",
			topic,
			partition,
			err,
		)
...
func (p *asyncProducer) bumpIdempotentProducerEpoch() {
	...
	if epoch == math.MaxInt16 {
		Logger.Println(
			"producer/txnmanager epoch exhausted, requesting new producer ID",
		)
...
func (p *asyncProducer) returnError(msg *ProducerMessage, err error) {
	...
	if !p.IsTransactional() && msg.hasSequence {
		Logger.Printf(
			"producer/txnmanager rolling over epoch due to publish failure on %s/%d",
			msg.Topic,
			msg.Partition,
		)
