...
func (r *DescribeUserScramCredentialsResponse) decode(
	pd packetDecoder,
	version int16,
) error {
	...
	if numUsers > 0 {
		...
		for i := 0; i < numUsers; i++ {
			...
			r.Results[i].CredentialInfos = make(
				[]*UserScramCredentialsResponseInfo,
				numCredentialInfos,
			)
			for j := 0; j < numCredentialInfos; j++ {
				...
				r.Results[i].CredentialInfos[j].Mechanism = ScramMechanismType(
					scramMechanism,
				)
