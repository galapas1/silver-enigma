...
func (r *AlterPartitionReassignmentsResponse) AddError(
	topic string,
	partition int32,
	kerror KError,
	message *string,
	...
	partitions[partition] = &alterPartitionReassignmentsErrorBlock{
		errorCode:    kerror,
...
func (r *AlterPartitionReassignmentsResponse) decode(
	pd packetDecoder,
	version int16,
) (err error) {
	r.Version = version
	...
	if numTopics > 0 {
		r.Errors = make(
			map[string]map[int32]*alterPartitionReassignmentsErrorBlock,
			numTopics,
		)
		for i := 0; i < numTopics; i++ {
			...
			r.Errors[topic] = make(
				map[int32]*alterPartitionReassignmentsErrorBlock,
