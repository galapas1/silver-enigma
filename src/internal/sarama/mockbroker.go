...
func (b *MockBroker) Close() {
	...
	if len(b.expectations) > 0 {
		buf := bytes.NewBufferString(
			fmt.Sprintf(
				"mockbroker/%d: not all expectations were satisfied! Still waiting on:\n",
				b.BrokerID(),
			),
		)
...
func (b *MockBroker) readToBytes(r io.Reader) ([]byte, error) {
	...
	if length <= 4 || length > MaxRequestSize {
		return nil, PacketDecodingError{
			fmt.Sprintf("message of length %d too large or too small", length),
		}
	}
...
func (b *MockBroker) handleRequests(
	...
	for {
		...
		if err != nil {
			if !isConnectionClosedError(err) {
				Logger.Printf(
					"*** mockbroker/%d/%d: invalid request: err=%+v, %+v",
					b.brokerID,
					idx,
					err,
					spew.Sdump(buffer),
		...
		if !b.isGSSAPI(buffer) {
			...
			resHeader := b.encodeHeader(
				res.headerVersion(),
				req.correlationID,
				uint32(len(encodedRes)),
			)
	...
	Logger.Printf(
		"*** mockbroker/%d/%d: connection closed, err=%v",
		b.BrokerID(),
		idx,
		err,
	)
...
func (b *MockBroker) encodeHeader(
	headerVersion int16,
	correlationId int32,
